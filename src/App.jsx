...
import { MapPin, Clock, Bell, ChevronLeft, Home, AlertTriangle, Navigation } from "lucide-react";
  ...
  return [
    {
      ...
      name: "竹橋経由 東西線ルート",
      steps: [
        { icon: "🚶", label: "徒歩 5分", note: "神保町 → 竹橋" },
        { icon: "🚆", label: "東西線→大手町乗換", note: "3駅・9分" },
        { icon: "🚆", label: "総武線快速 千葉行き", note: "東京 → 千葉・43分" },
      ],
      cost: 290, time: 57,
    ...
    },
    {
      ...
      name: "東京駅まで徒歩",
      steps: [
        { icon: "🚶", label: "徒歩 12分", note: "神保町 → 東京駅" },
        { icon: "🚆", label: "総武線快速 千葉行き", note: "東京 → 千葉・43分" },
      ],
      cost: 590, time: 65,
    ...
    },
    {
      ...
      name: "タクシー → 東京 → 総武線",
      steps: [
        { icon: "🚕", label: "タクシー 約8分", note: "神保町→東京駅 ¥1,200" },
        { icon: "🚆", label: "総武線快速 終電", note: "東京→千葉 43分 ¥590" },
      ],
      cost: 1790, time: 51,
    ...
    {
      id: "x2", mode: "taxi", badge: "終電後OK", badgeColor: C.purple,
      name: "タクシー直行（終電後でも可）",
      steps: [{ icon: "🚕", label: "タクシー 約50分", note: "神保町 → 千葉市中央区" }],
      cost: 9500, time: 50, lastTrain: null, depart: null,
      urgency: "safe", routeColor: C.purple,
    },
  ];
}
  ...
  return (
    <div style={{ borderRadius: 14, overflow: "hidden", position: "relative", height: h, background: "#080F1E" }}>
      <svg width="100%" height={h} viewBox="0 0 354 160" preserveAspectRatio="xMidYMid slice">
        ...
        <path d="M0,95 Q60,90 120,98 Q180,105 240,95 Q300,85 354,92" stroke="#0D2035" strokeWidth="8" fill="none" opacity="0.7" />
        ...
        <text x="82" y="124" fontSize="8" fill={C.accent} fontWeight="bold" opacity="0.9">神保町</text>
        <text x="295" y="52" fontSize="8" fill={C.blue} fontWeight="bold" opacity="0.9">東京</text>
      </svg>
...
// ── ホーム画面（自動アラーム付き）──────────────────────
function HomeScreen({ onShowRoutes }) {
  ...
  const routes = buildRoutes(currentNow);
  const recommended = routes[0]; // おすすめルート（電車・最速）

  const sec = secLeft(recommended.depart);
  const totalMin = recommended.depart ? recommended.depart.h * 60 + recommended.depart.m - (currentNow.h * 60 + currentNow.m) : null;
  ...
  const urgBg = sec === null ? C.green : sec <= 60 ? C.red : sec <= 300 ? C.amber : C.green;
  ...
  function fmtSec(s) {
    ...
    const r = s % 60;
    if (m > 0) return `あと ${m}分 ${String(r).padStart(2,"0")}秒`;
    return `あと ${r}秒！`;
  }
  ...
  if (fired) {
    return (
      <div style={{ position: "absolute", inset: 0, zIndex: 50, background: "linear-gradient(160deg,#220008,#150003)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, animation: "fadeIn 0.3s ease" }}>
        ...
        <button onClick={() => { setFired(false); setAlarmOn(false); }} style={{ width: "100%", padding: "16px", background: C.red, border: "none", borderRadius: 18, color: "#fff", fontWeight: 800, fontSize: 16, cursor: "pointer", boxShadow: `0 4px 24px ${C.red}60` }}>
          確認しました　→　出発！
        </button>
      </div>
    );
  ...
  return (
    <div style={{ padding: "18px 22px 32px", animation: "fadeIn 0.4s ease" }}>
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 10, color: C.sub, letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 6 }}>📍 神保町 → 自宅（千葉）</div>
        <h1 style={{ fontSize: 20, fontWeight: 900, color: C.white, margin: 0, lineHeight: 1.3 }}>ちゃんと送るからね</h1>
      </div>

      {/* メインカウントダウン */}
      <div style={{ ...cs({ marginBottom: 12, borderColor: `${urgBg}40`, background: `${urgBg}08`, textAlign: "center" }) }}>
        <div style={{ fontSize: 11, color: C.sub, marginBottom: 6, fontWeight: 600 }}>おすすめルートの出発まで</div>
        <div style={{ fontSize: 48, fontWeight: 900, color: urgColor, letterSpacing: -2, lineHeight: 1, marginBottom: 8, fontVariantNumeric: "tabular-nums" }}>
          {fmtSec(sec)}
        </div>
      ...
      <div style={cs({ marginBottom: 12 })}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
          <Pill color={recommended.badgeColor}>{recommended.badge}</Pill>
...
function RoutesScreen({ onBack, onAlarm }) {
  ...
  return (
    <div style={{ padding: "10px 18px 40px", animation: "fadeIn 0.3s ease" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
        <button onClick={onBack} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "8px 10px", cursor: "pointer", lineHeight: 0, display: "inline-flex", alignItems: "center" }}>
          <ChevronLeft size={18} color={C.white} />
        </button>
        <div>
          <div style={{ color: C.white, fontWeight: 800, fontSize: 15 }}>神保町 → 千葉</div>
          <div style={{ color: C.sub, fontSize: 11 }}>現在時刻: {now.toLocaleTimeString("ja-JP", { hour: "2-digit", minute: "2-digit", hour12: false })}</div>
        </div>
      </div>

      <div style={{ fontSize: 11, color: C.sub, fontWeight: 700, letterSpacing: 0.5, marginBottom: 8 }}>🚆 電車ルート</div>
      {trainRoutes.map(r => (
        <RouteCard key={r.id} route={r} onAlarm={onAlarm} selected={selected === r.id} onSelect={() => setSelected(r.id)} />
      ))}

      <div style={{ fontSize: 11, color: C.sub, fontWeight: 700, letterSpacing: 0.5, marginTop: 6, marginBottom: 8 }}>🚕 タクシー＋電車ルート</div>
      {taxiRoutes.map(r => (
        <RouteCard key={r.id} route={r} onAlarm={onAlarm} selected={selected === r.id} onSelect={() => setSelected(r.id)} />
      ))}
    </div>
...
function AlarmScreen({ route, onBack }) {
  ...
  if (fired) {
    return (
      <div style={{ position: "absolute", inset: 0, zIndex: 50, background: "linear-gradient(160deg,#220008,#150003)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, animation: "fadeIn 0.3s ease" }}>
        ...
        <div style={{ color: "#FF9090", fontSize: 14, textAlign: "center", lineHeight: 1.6, marginBottom: 8 }}>{route.name}</div>
        <div style={{ color: "#FF6060", fontSize: 13, textAlign: "center", lineHeight: 1.7, marginBottom: 40 }}>今すぐ出発しないと<br />終電を逃してしまいます</div>
        <button onClick={() => { setFired(false); setOn(false); }} style={{ width: "100%", padding: "16px", background: C.red, border: "none", borderRadius: 18, color: "#fff", fontWeight: 800, fontSize: 16, cursor: "pointer", boxShadow: `0 4px 24px ${C.red}60` }}>
          確認しました　→　出発！
        </button>
      </div>
    );
  ...
  return (
    <div style={{ padding: "10px 22px 40px", animation: "fadeIn 0.3s ease" }}>
      ...
      <div style={{ marginBottom: 12 }}>
        <MapPreview routeColor={route.routeColor || C.blue} compact />
      </div>
      ...
      <div onClick={() => setOn(v => !v)} style={cs({ cursor: "pointer", borderColor: on ? `${C.green}40` : C.border, transition: "border-color 0.3s" })}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#030608", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "20px 0 48px", fontFamily: "'Hiragino Sans','Yu Gothic UI','Meiryo','Noto Sans JP',sans-serif" }}>
        <div style={{ width: 390, background: C.bg, borderRadius: 48, border: "1px solid rgba(255,255,255,0.09)", overflow: "hidden", position: "relative", boxShadow: "0 40px 80px rgba(0,0,0,0.85)" }}>
          <StatusBar />
          {screen === "home" && <HomeScreen onShowRoutes={() => setScreen("routes")} />}
          {screen === "routes" && <RoutesScreen onBack={() => setScreen("home")} onAlarm={r => { setRoute(r); setScreen("alarm"); }} />}
